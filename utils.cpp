...
Utilities::Utilities(double w_info_gain, double w_travel_distance,
                     double w_sensing_time, double w_rfid_gain,
                     double w_battery_status) {
...
void Utilities::cleanPossibleDestination2(std::list<Pose> *possibleDestinations,
  ...
  if (possibleDestinations->size() > 0) {
    std::list<Pose>::iterator findIter = std::find(
        possibleDestinations->begin(), possibleDestinations->end(), p);
...
Pose Utilities::createFromInitialPose(int x, int y, int orientation,
                                      int variation, int range, int FOV) {
  Pose tmp = Pose(x, y, (orientation + variation) % 360, FOV, range);
  return tmp;
}

double Utilities::calculateDistance(list<Pose> history, dummy::Map *map,
                                    Astar *astar) {
  ...
  for (auto it = history.begin(); it != prev(history.end(), 1); it++) {
    ...
    // std::cout << it->getX() << " " << it->getY() << " : " << it2->getX() << "
    // " << it2->getY() << endl; std::cout << "1" << endl;
...
double Utilities::calculateRemainingBatteryPercentage(list<Pose> history,
                                                      dummy::Map *map,
                                                      Astar *astar) {
  // std::list<Pose>::iterator it = history.begin();
...
void Utilities::updatePathMetrics(int *count, Pose *target, Pose *previous,
  ...
  // position, without the current pose of the robot We don't want to visit this
  // cell again
  std::pair<string, list<Pose>> pair = make_pair(actualPose, *nearCandidates);
...
void Utilities::printResult(long newSensedCells, long totalFreeCells,
  ...
  std::cout
      << "-----------------------------------------------------------------"
      << endl;
  std::cout << "Area sensed: " << newSensedCells << " / " << totalFreeCells
            << " [" << 100 * (float)newSensedCells / (float)totalFreeCells
            << "%]" << endl;
  std::cout << "Total cell visited :" << numConfiguration << endl;
  std::cout << "Total travelled distance (cells): " << travelledDistance
            << endl;
  // std::cout << "Total travel time: " << travelledDistance / 0.5 << "s, " << (
  // travelledDistance/0.5) /60 << " m"<< endl; std::cout << "I came back to the
  // original position since i don't have any other candidate position"<< endl;
  // std::cout << "Total exploration time (s): " << travelledDistance / 0.5 <<
  // endl;
  ...
  // std::cout << "Total time for exploration: " << travelledDistance/0.5 +
  // totalScanTime << "s, " <<
...
// Usage example: filePutContents("./yourfile.txt", "content", true);
void Utilities::saveCoverage(const std::string &name,
                             const std::string &content, bool append) {
  std::ofstream outfile;
  std::ifstream pFile(name);
  if (outfile.fail()) {
    ...
    if (pFile.peek() == std::ifstream::traits_type::eof()) { // file is empty
      ...
      outfile.open(name);
      outfile << "w_info_gain,w_travel_distance,w_sensing_time,w_rfid_gain,w_"
                 "battery_status,numConfiguration,incrementalCoverage"
              << endl;
    } else {
      // std::cout << "File exists! Appending data!" << endl;
      outfile.open(name, std::ios_base::app);
...
bool Utilities::recordContainsCandidates(
    EvaluationRecords *record, int *count, Pose *target, Pose *previous,
    string *actualPose, list<Pose> *nearCandidates,
    vector<pair<string, list<Pose>>> *graph2, dummy::Map *map,
    MCDMFunction *function, list<Pose> *tabuList, vector<string> *history,
    int *encodedKeyValue, Astar *astar, long *numConfiguration,
    double *totalAngle, double *travelledDistance, int *numOfTurning,
    double *scanAngle, bool *btMode, double *threshold, RFID_tools *rfid_tools,
    double *batteryTime, bool *explorationCompleted) {
  ...
  if (!this->contains(*tabuList, *target)) {
    ...
    *encodedKeyValue = 1;
    this->updatePathMetrics(
        count, target, previous, *actualPose, nearCandidates, graph2, map,
        function, tabuList, history, *encodedKeyValue, astar, numConfiguration,
        totalAngle, travelledDistance, numOfTurning, *scanAngle, batteryTime);
  }
  // ...otherwise, if the selected cell has already been visited
    ...
    if (graph2->at(graph2->size() - 1).second.size() != 0) {
      ...
      if (record->size() != 0) {
        ...
        *encodedKeyValue = 2;
        ...
        this->updatePathMetrics(count, target, previous, *actualPose,
                                ...
                                numConfiguration, totalAngle, travelledDistance,
      ...
      // graph
        ...
        if (graph2->size() == 0)
          return true;
        // Otherwise, select as new position the last cell in the graph and then
    ...
    // its last pose
      ...
      *target = record->getPoseFromEncoding(targetString);
      ...
      // std::cout << "[BT2 - Tabulist]There are visible cells but the selected
      // one is already explored!Come back to two position ago"<< endl;
      count++;
    }
  }
  return false;
...
bool Utilities::recordNOTContainsCandidates(
    vector<pair<string, list<Pose>>> *graph2, EvaluationRecords *record,
    Pose *target, Pose *previous, vector<string> *history,
...
void Utilities::recordContainsCandidatesBT(
    EvaluationRecords *record, int *count, Pose *target, Pose *previous,
    string *actualPose, list<Pose> *nearCandidates,
    vector<pair<string, list<Pose>>> *graph2, dummy::Map *map,
    MCDMFunction *function, list<Pose> *tabuList, vector<string> *history,
  ...
  *target = result.first;
  ...
  if (!this->contains(*tabuList, *target)) {
    ...
    *btMode = false;
    ...
    // std::cout << "[BT-MODE4] Go back to previous positions in the graph" <<
    // endl;
  }
  // ... otherwise, if the cells has already been visited
    ...
    if (nearCandidates->size() != 0) {
      // std::cout << "2" << endl;
      // std::cout << "[BT-MODE1]Already visited, but there are other
      // candidates" << endl;
    ...
    // ...otherwise, if there are no more candidates
      ...
      // std::cout << "3" << endl;
      // std::cout << "[BT-MODE2] Go back to previous positions in the graph" <<
      // endl; Select as target the last element in the graph
      string targetString = graph2->at(graph2->size() - 1).first;
      *target = record->getPoseFromEncoding(targetString);
...
void Utilities::recordNOTContainsCandidatesBT(
    EvaluationRecords *record, int *count, Pose *target, Pose *previous,
    string *actualPose, list<Pose> *nearCandidates,
    vector<pair<string, list<Pose>>> *graph2, dummy::Map *map,
    MCDMFunction *function, list<Pose> *tabuList, vector<string> *history,
    int *encodedKeyValue, Astar *astar, long *numConfiguration,
    double *totalAngle, double *travelledDistance, int *numOfTurning,
    double *scanAngle, bool *btMode, double *threshold, double *batteryTime) {
  ...
  *encodedKeyValue = 2;
  this->updatePathMetrics(
      ...
      function, tabuList, history, *encodedKeyValue, astar, numConfiguration,
      totalAngle, travelledDistance, numOfTurning, *scanAngle, batteryTime);
  // Do not leave backtracking before finding a pose with a record.size() > 0
  // (new frontiers from there) *btMode = false; std::cout << "[BT-MODE3] Go
  // back to previous positions in the graph" << endl;
}

void Utilities::createMultiplePosition(
    list<Pose> *frontiers, vector<pair<long, long>> *candidatePosition,
    int range, double FOV) {
  vector<pair<long, long>>::iterator it = candidatePosition->begin();
  for (it; it != candidatePosition->end(); it++) {
    Pose p1 = Pose((*it).first, (*it).second, 0, range, FOV);
...
void Utilities::updateMaps(dummy::Map *map, Pose *target,
  ...
  for (int i = 0; i < rfid_tools->tags_coord.size(); i++) {
    ...
    // mfc prev
    // double rxPower = rfid_tools->rm->received_power_friis(relTagCoord.first,
    ...
    map->updatePathPlanningGrid(target->getX(), target->getY(),
                                target->getRange(),
                                rxPower - rfid_tools->sensitivity);
    ...
    rfid_tools->rm->addMeasurement(target->getX(), target->getY(),
                                   target->getOrientation(), rxPower, phase,
                                   rfid_tools->freq, tagLayerName);

    // mfc: dirty trick to plot sequential images of current prob maps
    // static int lineal_index = 0;
    // if ( i == 8 ){
    //   std::cout<<"\t- Tag [" << i << "] at rel position (" <<
    //   relTagCoord.first << ", " << relTagCoord.second << ") m. " <<std::endl;
    //   std::cout<<"\t- Reading at freq (" << *freq/1e6<< " MHz): (" <<
    //   (rxPower+30) << ") dBm. ( " << phase << ") rads. " << std::endl;

    //   rfid_tools->rm->saveProbMapDebug("/tmp/",i,lineal_index++,*x,*y,
    ...
    if (rxPower < rfid_tools->sensitivity) {
      ...
      rxPower = 1;
    rfid_tools->RFID_maps_list->at(i).addEllipse(
        rxPower, map->getNumGridRows() - target->getX(), target->getY(),
        target->getOrientation(), -1.0, target->getRange());
  }
}
...
void Utilities::computePosteriorBeliefSingleLayer(dummy::Map *map, Pose *target,
  ...
  rfid_tools->rm->addMeasurement(target->getX(), target->getY(),
                                 target->getOrientation(), rxPower, phase,
                                 rfid_tools->freq, "kl");
  // rfid_tools->rm->addTmpMeasurementRFIDCriterion(target->getX(),
  // target->getY(), target->getOrientation() , rxPower, phase,
...
bool Utilities::updateNavigationGraph(int *count, MCDMFunction *function,
  ...
  if (count == 0) {
    ...
    this->eastInitial =
        this->createFromInitialPose(*x, *y, *orientation, 90, *range, *FOV);
    this->westInitial =
        this->createFromInitialPose(*x, *y, *orientation, 270, *range, *FOV);
  ...
  // If it's not the first step but we are in one of the initial position (we
  // come back here with backtracking)
  if (count != 0 &&
    ...
    *actualPose = function->getEncodedKey(*target, 0);
    // Add to the graph the initial positions and the candidates from there
    // (calculated inside the function)
    this->pushInitialPositions(map, *x, *y, *orientation, *range, *FOV,
                               *threshold, *actualPose, graph2, function,
                               rfid_tools, batteryTime, explorationCompleted);
  }
  return false;
}

bool Utilities::forwardMotion(
    Pose *target, Pose *previous, list<Pose> *frontiers,
    list<Pose> *nearCandidates, vector<pair<long, long>> *candidatePosition,
    NewRay *ray, dummy::Map *map, long *x, long *y, int *orientation,
    double *FOV, int *range, vector<pair<string, list<Pose>>> *graph2,
    EvaluationRecords *record, MCDMFunction *function, double *threshold,
    int *count, vector<string> *history, list<Pose> *tmp_history,
    list<Pose> *tabuList, Astar *astar, double *imgresolution,
    double *travelledDistance, long *sensedCells, long *newSensedCells,
    long *totalFreeCells, double *totalScanTime, string *out_log,
    long *numConfiguration, string *actualPose, int *encodedKeyValue,
    double *totalAngle, int *numOfTurning, double *scanAngle, bool *btMode,
    RFID_tools *rfid_tools, double *accumulated_received_power,
    double *precision, double *batteryTime, bool *explorationCompleted) {
    ...
    *candidatePosition = ray->getCandidatePositions();
    ...
    if (graph2->size() > 1) {
      ...
      *nearCandidates = graph2->at(graph2->size() - 1).second;
      ...
      history->push_back(function->getEncodedKey(*target, 2));
      // std::cout << "[BT]No significative position reachable. Come back to
      // previous position" << endl;
      *count++;
      *btMode = true;
    }
    ...
    // The navigation is finished!
      ...
      // std::cout << "Travelled distance calculated during the algorithm: " <<
      // travelledDistance << endl; std::cout << "------------------ HISTORY
      // -----------------" << endl; Retrieve the cell visited only the first
      // time *tmp_history = this->cleanHistory(history, record);
      // this->calculateDistance(*tmp_history, map, astar );

      // std::cout << "------------------ TABULIST -----------------" << endl;
      // Calculate the path connecting the cells in the tabulist, namely the
      // cells that are visited one time and couldn't be visite again

      // NOTE Get correct values from tabuList
      *travelledDistance = this->calculateDistance(*tabuList, map, astar);
      *batteryTime =
          this->calculateRemainingBatteryPercentage(*tabuList, map, astar);

      // Normalise the travel distance in meter
      // NOTE: assuming that the robot is moving at 0.5m/s and the resolution of
      // the map is 0.5m per cell) if ( *imgresolution == 1.0 )
      // {
      ...
      this->printResult(*newSensedCells, *totalFreeCells, *precision,
                        ...
                        *accumulated_received_power, batteryTime);
      string content =
          to_string(this->w_info_gain) + "," +
          to_string(this->w_travel_distance) + "," +
          to_string(this->w_sensing_time) + "," + to_string(this->w_rfid_gain) +
  ...
  //pose of the robot
    ...
    *nearCandidates = record->getFrontiers();
    ...
    // ... otherwise, if there are no candidate positions
      ...
      // std::cout <<"F6" << endl;
      bool break_loop = this->recordNOTContainsCandidates(
          graph2, record, target, previous, history, function, count);
      if (break_loop == true)
        return true;
    }
...
double Utilities::findTags(vector<RFIDGridmap> *RFID_maps_list,
  ...
  // tag= map.findTag();
  // std::cout << "RFID pose: [" << tag.second << "," << tag.first << "]" <<
  // endl;
...
void Utilities::saveRFIDMapsWithGroundTruths(
    vector<RFIDGridmap> *RFID_maps_list,
    vector<pair<double, double>> *tags_coord, string root) {
  for (int i = 0; i < (*RFID_maps_list).size(); i++) {
    std::string path = root + to_string(i + 1) + ".pgm";
    (*RFID_maps_list)[i].saveAsWithGroundTruth(path, (*tags_coord)[i].first,
                                               (*tags_coord)[i].second);
  }
}

void Utilities::getEllipseSize(int X_max, int X_min, double *major_axis,
                               double *minor_axis) {
