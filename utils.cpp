...
void Utilities::updateMaps( dummy::Map* map, Pose* target,
  ...
  for (int i = 0; i < rfid_tools->tags_coord.size(); i++){
    ...
    //mfc not sure what do you want to do here, but we got rid of ellipses long ago RIC
    std::cerr << "RIC, if you get to use Utils.cpp, line "<< __LINE__ << "please consider fixing the commented line below ..."  << '\n';
    abort();
...
void Utilities::computePosteriorBeliefSingleLayer( dummy::Map* map, Pose* target,
  ...
  double rx_power  = rfid_tools->rm->received_power_friis_with_obstacles(target->getX(), target->getY(), target->getOrientation() * 3.141592/180.0,(rfid_tools->tags_coord)[tag_id].first, (rfid_tools->tags_coord)[tag_id].second, 0, rfid_tools->freq);
  //mfc: the above gets the received power between a robot in "target" in METERS and tags_coord[i] in METERS. I'm assuming orientation is in deg. And that your radio model has correct tx power defined

  double phase = rfid_tools->rm->phaseDifference(relTagCoord.first, relTagCoord.second, rfid_tools->freq);    
  //So, robot at pr (x,y,orientation) (long, long, int) receives rxPower,phase,freq from tag i . 
  //mfc looks like stuff from newModel branch got into noArea?
  std::cerr << "RIC, if you get to use Utils.cpp, line "<< __LINE__ << "please consider fixing the commented line below ..."  << '\n';
  abort();
  //rfid_tools->rm->addTmpMeasurementRFIDCriterion(target->getX(), target->getY(), target->getOrientation() , rx_power, phase, rfid_tools->freq, tag_id, len_update);
}
