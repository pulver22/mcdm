...
double RFIDCriterion::evaluateEntropyOverBelief(Pose &p, dummy::Map *map,
  ...
  for (int tag_id = 0; tag_id < rfid_tools->tags_coord.size(); tag_id++) {
    //mfc something from other branches got to noArea Branch, please RIC have a look at it ...
    // entropy_cell = rfid_tools->rm->getTotalEntropy(p.getX(), p.getY(), p.getOrientation(),buffer_size, buffer_size, tag_id);
    std::cerr << "RIC, if you get to use RFIDCriterion.cpp, line "<< __LINE__ << "please consider reincuding in RadarModel the commented line above ..."  << '\n';
    abort();
...
double RFIDCriterion::evaluateKLDivergence(Pose &p, dummy::Map *map,
  ...
  for (int tag_id = 0; tag_id < rfid_tools->tags_coord.size(); tag_id++) {
    ...
    //mfc something from other branches got to noArea Branch, please RIC have a look at it ...
    // KL_div_cell = rfid_tools->rm->getTotalKL(p.getX(), p.getY(), p.getOrientation(), buffer_size, buffer_size, tag_id);
    std::cerr << "RIC, if you get to use RFIDCriterion.cpp, line "<< __LINE__ << "please consider reincuding in RadarModel the commented line above ..."  << '\n';
    abort();
