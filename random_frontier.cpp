...
int main(int argc, char **argv) {
  ...
  do {
    ...
    if (btMode == false) {
      ...
      // If it's not the first step but we are in one of the initial position
      // (we come back here with backtracking)
      if (count != 0 &&
        ...
        utils.pushInitialPositions(&map, x, y, orientation, range, FOV,
                                   threshold, actualPose, &graph2, &function,
                                   &rfid_tools, &batteryTime, &explorationCompleted );
      }

      // If there are no new candidate positions from the current pose of the
      // robot
      ...
      //current pose of the robot
        ...
        // Evaluate the frontiers and return a list of <frontier, evaluation>
        // pairs
        ...
        if (record->size() != 0) {
          ...
          // If the selected destination does not appear among the cells already
          // visited
          ...
          // ...otherwise, if the seleced cell has already been visited
            ...
            if (graph2.size() == 0)
              break;
            // If there still are more candidates to explore from the last pose
            // in the graph
            if (graph2.at(graph2.size() - 1).second.size() != 0) {
              ...
              if (record->size() != 0) {
                ...
                utils.updatePathMetrics(
                    ...
                    encodedKeyValue, &astar, &numConfiguration, &totalAngle,
                    &travelledDistance, &numOfTurning, scanAngle, &batteryTime);
                // Set that we are now in backtracking
                btMode = true;
              }
              // If there are no more candidate position from the last position
              // in the graph
              else {
                // if the graph is now empty, stop the navigation
                if (graph2.size() == 0)
                  break;
                // Otherwise, select as new position the last cell in the graph
  ...
  double belief_accuracy =
      utils.findTags(&RFID_maps_list, &tags_coord, &map, detection_log,
                     accuracy_log, initRange, numConfiguration, &rfid_tools, distance_log_path);
  cout << "-----------------------------------------------------------------"
       << endl;
...
Pose getRandomFrontier(list<Pose> nearCandidates) {
  ...
  std::uniform_int_distribution<int> uni(0, tot_candidates -
