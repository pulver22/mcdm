...
for motion_index in range(len(std_dev_list)):
    ...
    for i in range(0,run):
        tmp = np.loadtxt(open(root + 'std__' + str(motion) + '/entropy_' + str(i) + '/entropy.csv', "rb"), delimiter=",")
        if (tmp.size > 1):
    ...
    normaliseDataAndPlot(ax, total_entropies, color=color_list[motion_index], label='$\sigma$ = {}'.format(motion))
    total_entropies.clear()

plt.legend(title="Std-dev values")
plt.xlabel('Robot configurations')
plt.ylabel('Entropy' )
plt.title('Map Entropy with moving tags')
# plt.show()
plt.tight_layout()
plt.savefig(root + 'entropy_vs_stddev.png', dpi=300)
