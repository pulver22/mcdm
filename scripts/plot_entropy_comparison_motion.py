...
for kernel in kernel_list:
    root = path + 'kernel_' + str(kernel) + 'x' + str(kernel) + '/'
    ...
    for motion_index in range(len(motion_list)):
        ...
        for i in range(0,run):
            tmp = np.loadtxt(open(root + str(motion) + '/entropy_' + str(i) + '/entropy.csv', "rb"), delimiter=",")
            coverage = np.loadtxt(open(root + str(motion) + coverage_log + str(i) + '.csv', "rb"), delimiter=",", skiprows=1)
        ...
        normaliseDataAndPlot(ax, total_entropies, color=color_list[motion_index], label=str(motion))
        total_entropies.clear()

    plt.legend(title="Std-dev values")
    plt.xlabel('Robot configurations')
    plt.ylabel('Entropy' )
    plt.title('Fixed kernel motion (' + str(kernel) +') vs Gaussian std-dev')
    # plt.show()
    plt.tight_layout()
    plt.savefig(path + 'kernel' + str(kernel) + '_vs_stddev.png', dpi=300)
