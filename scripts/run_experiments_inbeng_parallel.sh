...
do
    ...
    do
        ...
        ./../build/mcdm_online_exploration ./../Images/inbeng_small.pgm 1 ${X[$counter_run]} ${Y[$counter_run]} 180 9 180 0.99 0 1 ./../config/tag_inbeng_${counter_run}.yaml 902e6 -10 $w_info_gain $w_travel_distance $w_sensing_time $w_rfid_gain $w_battery_status ${out_path}/result_inbeng_mcdm_r$counter_run.csv ${out_path}/coverage_inbeng_mcdm_${i}_r${counter_run}.csv ${out_path}/distance_inbeng_mcdm_${i}_r${counter_run}.csv 1 ${out_path}/accuracy_inbeng_mcdm_r$counter_run.csv $use_mcdm &>/dev/null &
        ((counter++))
