...
do
    ...
    do
        ...
        ./../build/mcdm_online_exploration ./../Images/ncfm_small.pgm 1 ${X[$counter_run]} ${Y[$counter_run]} 180 12 180 0.99 0 1 ./../config/tag_ncfm_${counter_run}.yaml 902e6 -10 $w_info_gain $w_travel_distance $w_sensing_time $w_rfid_gain $w_battery_status ${out_path}/result_ncfm_mcdm_r$counter_run.csv ${out_path}/coverage_ncfm_mcdm_${i}_r${counter_run}.csv ${out_path}/distance_ncfm_mcdm_$r${counter_run}.csv 1 ${out_path}/accuracy_ncfm_mcdm_r$counter_run.csv $use_mcdm &>/dev/null &
        ((counter++))
